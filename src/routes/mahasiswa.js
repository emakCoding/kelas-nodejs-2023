...
router.post("/create", async (req, res) => {
  ...
  connection.execute("SELECT * FROM mahasiswa WHERE nim = ?", [nim], function (errors, rows) {
    if (errors) {
      console.log(errors);
    ...
    if (rows.length > 0) {
      return res.status(200).json({ status: "error", message: "NIM sudah digunakan." });
    }

    connection.execute("INSERT INTO mahasiswa (nim, nama, jenis_kelamin, prodi, jurusan) VALUES (?, ?, ?, ?, ?)", [nim, nama, jenis_kelamin, prodi, jurusan], function (errors, _) {
      if (errors) {
        console.log(errors);
        return res.status(500).json({
          status: "error",
